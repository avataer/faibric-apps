    ...
    <section className="py-12 px-4 bg-gray-50 min-h-screen">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-lg p-6 mb-12">
          <h3 className="text-xl font-bold text-gray-900 mb-4">Your Watchlist Summary</h3>
              ...
              <tbody>
                {stocks.map(stock => (
                  <tr key={stock.symbol} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4 font-bold text-indigo-600">{stock.symbol}</td>
                    <td className="py-3 px-4 text-gray-700">{stock.name}</td>
                    <td className="py-3 px-4 text-right font-semibold">${stock.price}</td>
                    <td className={`py-3 px-4 text-right ${stock.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {stock.change >= 0 ? '+' : ''}{stock.change}
                    </td>
                    <td className={`py-3 px-4 text-right ${stock.change >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-sm ${stock.change >= 0 ? 'bg-green-100' : 'bg-red-100'}`}>
...
const FAIBRIC_SITE_URL = "https://app-254-i-want-a-dashboard.onrender.com";
